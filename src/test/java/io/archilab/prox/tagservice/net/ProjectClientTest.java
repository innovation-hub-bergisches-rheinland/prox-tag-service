...
class ProjectClientTest {
  ...
  void setUp() throws Exception {
    ...
    this.wireMockServer = new WireMockServer(options().dynamicPort().extensions(new ResponseTemplateTransformer(false)));

    this.wireMockServer.stubFor(get(urlPathEqualTo("/")).willReturn(aResponse()
    ...
    this.wireMockServer.stubFor(get(urlPathEqualTo("/projects")).willReturn(aResponse()
        .withHeader(HttpHeaders.CONTENT_TYPE, MediaTypes.HAL_JSON_VALUE)
        .withBody(readFromFile("classpath:api/project-service/projects/projects.json"))
        .withTransformers("response-template")
    ));

    this.wireMockServer.stubFor(get(urlPathEqualTo("/projects/search")).willReturn(aResponse()
        .withHeader(HttpHeaders.CONTENT_TYPE, MediaTypes.HAL_JSON_VALUE)
        .withBody(readFromFile("classpath:api/project-service/projects/search/search.json"))
        .withTransformers("response-template")
    ));
    ...
    when(eurekaClient.getNextServerFromEureka(eq(PROJECT_SERVICE_ID), anyBoolean())).thenReturn(instanceInfo);
  ...
  void when_get_creator_id_of_project_valid_then_found() throws Exception {
    this.wireMockServer.stubFor(get(urlPathEqualTo("/projects/search/findAllByIds")).withQueryParam("projectIds", matching(UUID_REGEX)).willReturn(aResponse()
        .withHeader(HttpHeaders.CONTENT_TYPE, MediaTypes.HAL_JSON_VALUE)
        .withBody(readFromFile("classpath:api/project-service/projects/search/findAllByIds/singleProjectFound.json"))
        .withTransformers("response-template")
    ));
  ...
  void when_get_creator_id_of_project_invalid_then_not_found() throws Exception {
    this.wireMockServer.stubFor(get(urlPathEqualTo("/projects/search/findAllByIds")).withQueryParam("projectIds", matching(UUID_REGEX)).willReturn(aResponse()
        .withHeader(HttpHeaders.CONTENT_TYPE, MediaTypes.HAL_JSON_VALUE)
        .withBody(readFromFile("classpath:api/project-service/projects/search/findAllByIds/noProjectFound.json"))
        .withTransformers("response-template")
    ));
